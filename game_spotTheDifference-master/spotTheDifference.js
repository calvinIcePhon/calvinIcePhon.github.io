...
var spotTheDifference = {
	...
	,"stageInfosIdx":-1
	,"contnets":null
	,"stat":0 //0:게임전(게임오버), 1:로딩,2:게임중  0: Game over, 1: Loading, 2: In game.
	,"isDebug":true
	...
	/**
	* 초기이미지로 돌린다.(게임 끝났을 때 동작)  Display the initial image. (Action when the game is over)
	*/
	,"intImgs":function(){
	...
	,"setStageInfo":function(stageInfo){ //단일 동작, 옛날버전 호환용   //Single operation, compatible with old versions
	...
	,"solve":function(){
		if(!this.stageInfo) return false;//스테이지가 로드된 후에만 동작  Action only after the stage is loaded.
	...
	,"startGame":function(){
		if(this.stageInfos.length > this.stageInfosIdx){
			...
			this.gameclear(); //게임 클리어 Game cleared.
		}
	}
	//게임을 성공적으로 완료 Game successfully completed.
	,"gameclear":function(){
		var ts = [];
		var sumT = 0;
	...
	,"startTimer":function(){
		...
		console.log(limitTime + ' time here');
		this.gameTimer.max = 50;
	...
	,"onclickImgLR":function(evt){
		...
		var x = evt.offsetX!=undefined?evt.offsetX:evt.pageX-$(evt.target).offset().left; //FF에서는 offsetX,Y가 없다. There is no offsetX, Y in FF (Firefox).
		var y = evt.offsetY!=undefined?evt.offsetY:evt.pageY-$(evt.target).offset().top;

		this.checkAnswer(x,y);
	}
	/**
	* return : 0:못찾음, 1:찾음, 2:중복찾음  return: 0 - Not found, 1 - Found, 2 - Duplicate found.
	*/
	...
	,"drawCanvasCheckedPos":function(){
		...
		for(var i=0,m = sls.length;i<m;i++){
			...
			var pos = sls[i].poss[0]; //첫번째 위치만 표시한다.  //Display only the first location.
	...
	,"drawCanvasPos":function(canvas,pos,strokeStyle){
		...
		context2d.fillStyle = "rgba(255,255,255,0.5)"; //채우기 색  //Fill color.
		if(strokeStyle){
			context2d.strokeStyle = strokeStyle; //선색  Line color.
		}else{
	...
	,"parseDiffRange":function(){
		...
		for(var i=0,m=imgDataD.data.length;i<m;i+=4){
			if(imgDataD.data[i] < 1 &&imgDataD.data[i+3]>0 ){
				...
				if(rarr.length < 50){continue;} //10픽셀 이상 차이가 있어야한다.  There should be a difference of at least 10 pixels.
				//alert(rarr.length);
				solutions.push(rarr);
			}
		}
		console.log('here ' + solutions);
		//-- 최대x,y 최소x,y를 구해서 원을 그림   -- Find the maximum x, y and minimum x, y and draw a circle
		var sls = []
		...
		for(var i=0,m=solutions.length;i<m;i++){
			...
			for(var i2=0,m2=rarr.length;i2<m2;i2++){
				...
				y0 = Math.min(y0,y);
				y1 = Math.max(y1,y);
				imgDataD.data[posIdx] = (i*100)%250; //그룹에 맞춰서 색바꾸기  //Change color according to the group.
			}
			//alert(x0+","+x1+","+y0+","+y1)
		...
		*/
		...
		for(var i=0;i<sls.length;i++){
			...
			for(var i2=sls.length-1;i2>i;i2--){
				...
				if(r2<= pos0['r']+pos2['r']){
					if(r2 <= pos0['r']){ //속에 포함될 때 When it is included inside.
		...
		//*/
		//--- title들을 재정의   Redefine the titles.
		var minR = this.imgL.naturalWidth/20; //원의 최소 너비  Minimum width of the circle.
		for(var i=0;i<sls.length;i++){
			var sl  = sls[i];
			sl.id="pos"+(i+1);
			sl.title="틀린부분"+(i+1);
	...
	,"searchDiffRange":function(imgDataD,posIdx){
		...
		while(shArr.length>0){
			...
			if(data[posIdx]>0){continue;} //이미 체크한 것이다. It's already checked.
			//console.log(posIdx+":"+data[posIdx+3]+":"+data[posIdx]);
			rarr.push(posIdx);
			data[posIdx]+=100; //체크했다고 표시한다. Mark it as checked.
